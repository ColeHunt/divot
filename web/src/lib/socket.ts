...
/**
 * A WebSocket that reconnects with backoff and re-subscribes to its round
 * automatically. Auth rides the session cookie sent with the upgrade
 * request, so there is no hello payload to carry credentials in.
 */
export class RoundSocket {
  ...
  constructor(
    ...
    private readonly handlers: RoundSocketHandlers,
  ) {}
  ...
  private openSocket(): void {
    ...
    socket.onopen = () => {
      ...
      socket.send(JSON.stringify({ t: 'subscribe', code: this.code }));
  ...
  private scheduleReconnect(): void {
    ...
    // Jitter keeps a whole foursome's phones from reconnecting in lockstep.
